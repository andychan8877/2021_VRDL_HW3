...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 hw3_nuclei.py train --dataset=/path/to/dataset --subset=train --weights=/path/to/weights.h5

    # Resume training a model that you had trained earlier
    python3 hw3_nuclei.py train --dataset=/path/to/dataset --subset=train --weights=last

    # Generate submission file
    python3 hw3_nuclei.py detect --dataset=/path/to/dataset --subset=train --weights=<last or /path/to/weights.h5>
"""
...
# 要先把pretrained的weight也放在Mask RCNN的主路徑
...
class NucleiConfig(Config):
    """Configuration for training on the nuclei segmentation dataset."""
    ...
    # then 0.5 is the minimum anyway as it picks between nuclei and BG
...
class NucleiDataset(utils.Dataset):
    ...
    def load_nuclei(self, dataset_dir, subset):
        ...
        # Naming the dataset nuclei, and the class nuclei
        ...
        assert subset in ["train", "val", "train_all", "test"]
...
def train(model, dataset_dir, subset):
    ...
    dataset_train.load_nuclei(dataset_dir, subset)
    dataset_train.prepare()

    # Validation dataset
    dataset_val = NucleiDataset()
    dataset_val.load_nuclei(dataset_dir, "val")
    dataset_val.prepare()
...
#             for j in range(len(pictDat[i]['height'])):
                ...
                figure = dict(image_id=img_id, bbox=figure_bbox, score=float(r['scores'][i]), category_id=int(r['class_ids'][i]))                     
#                 print(submission[0][i])           , segmentation=submission[0][i]
                print(type(img_id))
                print(type(r['class_ids'][i]))
                print(type(figure))
                print(figure)
